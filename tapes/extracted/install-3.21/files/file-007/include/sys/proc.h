/* @[$]proc.h	4.2  06/11/84 12:31:11 - Zilog Inc */

/*
 * One structure allocated per active process. It contains all data needed about
 * the process while the process may be swapped out. Other per process data 
...
struct	proc 
{
	...
	short		p_uid;		/* user id, to direct tty signals     */
	...
	struct proc	*p_rlink;	/* back pointer for same list 	      */
	struct rq	*p_rqh;		/* ptr to run queue header	      */
...
# define SIDL		4		/* in midst of process creation       */
# define SZOMB		5		/* in midst of process termination    */
...
/*
 * parallel proc structure to replace part with times to be passed to parent 
...
struct	xproc {
	...
	short	xp_uid;			/* user id, to direct tty signals     */
