...
/*
 * Source reconstruction of Zilog init.c 2.6 (08/20/84), based on the
 * stripped ZEUS 3.21 /etc/INIT binary.  The process-management core is the
 * System III source; the marked Zilog additions below are recovered from
 * the binary's data and control flow.
 */
...
/*
 * Zilog addition: init supplies a minimal environment to /etc/rc and every
 * inittab child.  The padding in TERM is present in the shipped binary and
 * leaves room for the terminal code read from /etc/ttytype.
 */
char *initenv[] = {
	"HOME=/",
...
/* Static buffers used by the Zilog tty-type lookup routines. */
char tcode[8];
char dname[20];
...
char **argv;
{
	...
	/*
	 * Zilog addition: determine the console's terminal type, then replace
	 * the inherited environment with the five-entry environment above.
	 */
	strcpy(initenv[2]+5, stypeof(DIAG));
	environ = initenv;

	/* The shipped binary selects state 2 when invoked with an m-name. */
	if(**argv == 'm')
		state = '2';
	...
	for(;;) {
		...
		/*
		 * Zilog addition: on the 1 -> 2 transition, terminate the old
		 * console process group before starting the state-2 entry.
		 */
		if((state == '2') && (lstate == '1')) {
			i = lookid("co");
			kill(-itab[i].in_proc, 15);
			sleep(1);
			i = lookid("co");
			kill(-itab[i].in_proc, 9);
		}
...
/*
 * Zilog additions.  These two routines correspond to the functions at
 * 0x0dc8 and 0x0e28 in the stripped ZEUS 3.21 init image.
 */
char *
_ttytype(name)
char *name;
{
	...
	/*
	 * This unconditional fclose is intentional: it is what the original
	 * binary does, including when fopen returned NULL.
	 */
	fclose(fd);
	return("un");
}
...
char *ttyid;
{
	return(_ttytype(strrchr(ttyid, '/')+1));
}
