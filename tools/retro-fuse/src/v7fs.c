...
int v7fs_mkfs(uint32_t fssize, uint32_t isize, const struct v7fs_flparams *flparams)
{
    ...
    fp->s_isize = wswap_int16(isize + 2); /* adjusted to point to the block beyond the i-list */
    fp->s_fsize = wswap_int32(fssize);
    fp->s_n = wswap_int16((int16_t)fn);
    fp->s_m = wswap_int16((int16_t)fm);
    fp->s_tfree = 0;
    fp->s_tinode = 0;
    ...
    for (v7_daddr_t iblk = 0; iblk < isize; iblk++) {
        ...
        v7_clrbuf(bp);
        /*
         * The on-disk superblock is byte-swapped on little-endian hosts.
         * Keep the running total in disk byte order just like alloc.c does;
         * direct arithmetic here previously wrote an impossible s_tinode
         * value (for example 47808 instead of 3931 on the 15000-block /usr
         * image), which makes the native ZEUS fsck distrust the inode map.
         */
        fp->s_tinode =
            wswap_int16((uint16_t)(wswap_int16(fp->s_tinode) + NIPB));
        if (iblk == 0) {
            struct v7_dinode * ip = (struct v7_dinode *)bp->b_un.b_addr;
            /* bad block file */
            ip->di_mode = wswap_int16((int16_t)(IFREG));
            ip->di_nlink = 0;
            ip->di_uid = 0;
            ip->di_gid = 0;
            ip->di_size = 0;
            ...
            /* root directory */
            ip++;
            ip->di_mode = wswap_int16((int16_t)(IFDIR|0777));
            ip->di_nlink = wswap_int16(2);
            ip->di_uid = wswap_int16(v7_u.u_uid);
            ip->di_gid = wswap_int16(v7_u.u_gid);
            ip->di_size = wswap_int32(2*sizeof(struct v7_direct)); /* size of 2 directory entries */
            ip->di_addr[0] = (char)(rootdirblkno >> 16);
            ip->di_addr[1] = (char)(rootdirblkno >> 8);
            ip->di_addr[2] = (char)(rootdirblkno);
            ip->di_atime = ip->di_mtime = ip->di_ctime = wswap_int32(v7_time);

            fp->s_tinode =
                wswap_int16((uint16_t)(wswap_int16(fp->s_tinode) - 2));
        }
        v7_bwrite(bp);
        if ((bp->b_flags & B_ERROR) != 0)
            goto exit;
     ...
     * the free table. */
    for (int i = wswap_int16(fp->s_nfree); i < NICFREE; i++)
        fp->s_free[i] = 0;
...
int v7fs_open(const char *name, int flags, mode_t mode)
{
    ...
    /* When creating, call namei() in "create" mode (flag=1) so that it
     * establishes u.u_pdir (the parent directory) even when the parent is
     * the root directory.  With flag=0 namei() never sets u.u_pdir, which
     * left it NULL for a first create directly under "/" and crashed
     * maknode() (u.u_pdir->i_dev).  Mirrors v7fs_mknod()/v7fs_mkdir(). */
    ip = v7_namei(&v7_schar, (flags & O_CREAT) ? 1 : 0);
    if (ip == NULL) {
        if ((flags & O_CREAT) == 0)
            return -v7_u.u_error;
        /* flag=1 leaves u_error==0 and u.u_pdir set when the final
         * component does not exist; any nonzero error is a real failure
         * (e.g. no write permission on the parent). */
        if (v7_u.u_error != 0)
            return -v7_u.u_error;
...
int v7fs_enumdir(const char *pathname, v7fs_enum_dir_funct enum_funct, void *context)
{
    ...
    while (1) {
        ...
        v7_u.u_error = 0;
        ip = v7_iget(v7_rootdev, wswap_int16(direntrybuf.direntry.d_ino));
        if (ip == NULL) {
...
int v7fs_statfs(const char *pathname, struct statvfs *statvfsbuf)
{
    ...
    statvfsbuf->f_files = (wswap_int16(fp->s_isize) - 2) * INOPB;
     ...
     */
    {
        int16_t nfree = wswap_int16(fp->s_nfree);
    ...
    for (v7_daddr_t blk = 2; blk < ((v7_daddr_t)wswap_int16(fp->s_isize)); blk++) {
    ...
    statvfsbuf->f_favail = statvfsbuf->f_ffree;
...
static void v7fs_initfreelist(struct v7_filsys *fp, uint16_t n, uint16_t m)
{
    ...
    for (; d > 0; d -= n)
        for (i=0; i<n; i++) {
            f = d - adr[i];
            if (f < wswap_int32(fp->s_fsize) && f >= wswap_int16(fp->s_isize))
...
static int v7fs_reparentdir(const char *pathname)
{
    ...
    if (wswap_int16(direntry.d_ino) != newparentip->i_number) {

        /* get the inode for the old parent directory. */
        oldparentip = v7_iget(newparentip->i_dev, wswap_int16(direntry.d_ino));
        if (oldparentip == NULL) {
        ...
        direntry.d_ino = wswap_int16(newparentip->i_number);
