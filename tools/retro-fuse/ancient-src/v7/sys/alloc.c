...
alloc(dev_t dev)
{
	...
	do {
		if(wswap_int16(fp->s_nfree) <= 0)
			goto nospace;
		if (wswap_int16(fp->s_nfree) > NICFREE) {
			prdev("Bad free count", dev);
			goto nospace;
		}
		fp->s_nfree = wswap_int16(wswap_int16(fp->s_nfree) - 1);
		bno = wswap_int32(fp->s_free[wswap_int16(fp->s_nfree)]);
		if(bno == 0)
			goto nospace;
	...
	bp = getblk(dev, bno);
	clrbuf(bp);
	if(wswap_int32(fp->s_tfree)) fp->s_tfree = wswap_int32(wswap_int32(fp->s_tfree) - 1);
	fp->s_fmod = 1;
	return(bp);
...
free(dev_t dev, daddr_t bno)
{
	...
	fp->s_free[wswap_int16(fp->s_nfree)] = wswap_int32(bno);
	fp->s_nfree = wswap_int16(wswap_int16(fp->s_nfree) + 1);
...
badblock(struct filsys *fp, daddr_t bn, dev_t dev)
{
	...
	if (bn < wswap_int16(fp->s_isize) || bn >= wswap_int32(fp->s_fsize)) {
		prdev("bad block", dev);
		return(1);
	}
	return(0);
...
loop:
	if(wswap_int16(fp->s_ninode) > 0) {
		fp->s_ninode = wswap_int16(wswap_int16(fp->s_ninode) - 1);
		ino = wswap_int16(fp->s_inode[wswap_int16(fp->s_ninode)]);
		if (ino < ROOTINO)
			goto loop;
		ip = iget(dev, ino);
		if(ip == NULL)
		...
		if(ip->i_mode == 0) {
			for (i=0; i<NADDR; i++)
				ip->i_un.i_addr[i] = 0;
			if(wswap_int16(fp->s_tinode)) fp->s_tinode = wswap_int16(wswap_int16(fp->s_tinode) - 1);
			fp->s_fmod = 1;
			return(ip);
		}
		/*
	...
	fp->s_ilock++;
	ino = 1;
	for(adr = SUPERB+1; adr < wswap_int16(fp->s_isize); adr++) {
		bp = bread(dev, adr);
		...
		for(i=0; i<INOPB; i++) {
			...
			if(dev==ip->i_dev && ino==ip->i_number)
				goto cont;
			fp->s_inode[wswap_int16(fp->s_ninode)] = wswap_int16(ino);
			fp->s_ninode = wswap_int16(wswap_int16(fp->s_ninode) + 1);
			if(wswap_int16(fp->s_ninode) >= NICINOD)
				break;
		cont:
			ino++;
			dp++;
...
ifree(dev_t dev, ino_t ino)
{
	...
	fp = getfs(dev);
	fp->s_tinode = wswap_int16(wswap_int16(fp->s_tinode) + 1);
	if(fp->s_ilock)
		return;
	if(wswap_int16(fp->s_ninode) >= NICINOD)
		return;
...
getfs(dev_t dev)
{
	...
	if(mp->m_bufp != NULL && mp->m_dev == dev) {
		...
		if(wswap_int16(fp->s_nfree) > NICFREE || wswap_int16(fp->s_ninode) > NICINOD) {
			prdev("bad count", dev);
			fp->s_nfree = 0;
			fp->s_ninode = 0;
		}
