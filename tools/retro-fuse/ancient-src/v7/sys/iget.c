...
iexpand(struct inode *ip, struct dinode *dp)
{
	...
	ip->i_mode = wswap_int16(dp->di_mode);
	ip->i_nlink = wswap_int16(dp->di_nlink);
	ip->i_uid = wswap_int16(dp->di_uid);
	ip->i_gid = wswap_int16(dp->di_gid);
...
iupdat(struct inode *ip, time_t *ta, time_t *tm)
{
	...
	if((ip->i_flag&(IUPD|IACC|ICHG)) != 0) {
		...
		dp = bp->b_un.b_dino;
		dp += itoo(ip->i_number);
		dp->di_mode = wswap_int16(ip->i_mode);
		dp->di_nlink = wswap_int16(ip->i_nlink);
		dp->di_uid = wswap_int16(ip->i_uid);
		dp->di_gid = wswap_int16(ip->i_gid);
...
wdir(struct inode *ip)
{
	...
	u.u_dent.d_ino = wswap_int16(ip->i_number);
